...
function solution(){
    ...
    return function(str){
        ...
        if(command === 'prepare'){
            ...
            let menuIngrs = Object.entries(menu[ingr]);
            // remove those from the ingredients obj
            let areAllmenuIngrs = true;
            for(let i=0; i<menuIngrs.length; i++){
                let menuIngredientQuantity = Number(menuIngrs[i][1]);
                let menuIngredientName = menuIngrs[i][0];
                if(ingredients[menuIngredientName] - (menuIngredientQuantity * Number(amt)) < 0){
                    areAllmenuIngrs = false; 
        ...
        if(command === 'report'){
            ...
            for(let ingr in ingredients){
                res.push(`${ingr}=${ingredients[ingr]}`);
            }
            return res.join(' ');
        }     
    }
