...
// rating is hot. If there is no majority, but the balance is non-negative and the sum of both votes are more than 100,
// its rating is controversial. If the balance is negative, the rating becomes unpopular. If the post has less than 10
// total votes, or no other rating is met, it’s rating is new regardless of balance. These calculations are performed on
// the actual numbers.
// Your function will be invoked with call(object, arguments), so treat it as though it is internal for the object. A
// forum post, to which the function will be attached, has the following structure:
// {
//     id: 'id',
//     author: 'author name',
//     content: 'text',
...
// The post begins at 100/100, we add one upvote and one downvote, bringing it to 101/101. The reported score is
// inflated by 25% of the greater value, rounded up (26). The balance is 0, and at least one of the numbers is greater
// than 100, so we return an array with rating 'controversial'.
// We downvote 50 times, bringing the score to 101/151, the reported values are inflated by 151*0.25=38 (rounded
// up) and since the balance is negative with return an array with rating 'unpopular'.